...
var useChartAnimation = ({
  ...
  const disableAnimation = React13.useCallback(() => {
...
var useChartInteractionListener = ({
  ...
  const addInteractionListener = React15.useCallback((interaction, callback, options) => {
    const svg = svgRef.current;
...
function useCharts(inPlugins, props, seriesConfig5) {
  const chartId = useId();
  const plugins = React16.useMemo(() => [...CHART_CORE_PLUGINS, ...inPlugins], [inPlugins]);
  const pluginParams = extractPluginParamsFromProps({
    plugins,
    props
  });
  pluginParams.id = pluginParams.id ?? chartId;
  const instanceRef = React16.useRef({});
  const instance = instanceRef.current;
  const publicAPI = useChartApiInitialization(props.apiRef);
  const innerChartRootRef = React16.useRef(null);
  const innerSvgRef = React16.useRef(null);
  const storeRef = React16.useRef(null);
...
function useChartApiInitialization(inputApiRef) {
  const fallbackPublicApiRef = React16.useRef({});
  if (inputApiRef) {
...
var useInteractionItemProps = (data, skip) => {
  ...
  const interactionActive = React19.useRef(false);
...
var useChartPolarAxis = ({
  ...
  const svg2rotation = React20.useMemo(() => generateSvg2rotation({
    cx: center.cx,
    cy: center.cy
  }), [center.cx, center.cy]);
  const svg2polar = React20.useMemo(() => generateSvg2polar({
    cx: center.cx,
    cy: center.cy
  }), [center.cx, center.cy]);
  const polar2svg = React20.useMemo(() => generatePolar2svg({
    cx: center.cx,
    cy: center.cy
  }), [center.cx, center.cy]);
...
function useAnimateInternal(props, {
  ...
  const lastInterpolatedPropsRef = React21.useRef(initialProps);
  const transitionRef = React21.useRef(null);
  const elementRef = React21.useRef(null);
  const lastPropsRef = React21.useRef(props);
  useEnhancedEffect_default(() => {
  ...
  const animate = React21.useCallback((element) => {
  ...
  const setRef = React21.useCallback((element) => {
    var _a, _b, _c, _d;
    if (element === null) {
...
function useChartGradientIdBuilder() {
  ...
  return React22.useCallback((axisId) => `${chartId}-gradient-${axisId}`, [chartId]);
}
function useChartGradientIdObjectBoundBuilder() {
  const chartId = useChartId2();
  return React22.useCallback((axisId) => `${chartId}-gradient-${axisId}-object-bound`, [chartId]);
}
  ...
  return (0, import_jsx_runtime23.jsxs)("defs", {
    children: [filteredYAxisIds.map((axisId) => {
      ...
      if ((colorMap == null ? void 0 : colorMap.type) === "continuous") {
        return (0, import_jsx_runtime23.jsxs)(React48.Fragment, {
...
var ToolbarContext = React50.createContext(void 0);
if (true) ToolbarContext.displayName = "ToolbarContext";
function useToolbarContext() {
  const context = React50.useContext(ToolbarContext);
  if (context === void 0) {
...
function ToolbarContextProvider({
  ...
  const [focusableItemId, setFocusableItemId] = React50.useState(null);
  const focusableItemIdRef = React50.useRef(focusableItemId);
  const [items, setItems] = React50.useState([]);
  const getSortedItems = React50.useCallback(() => items.sort(sortByDocumentPosition), [items]);
  const findEnabledItem = React50.useCallback((startIndex, step, wrap = true) => {
    var _a, _b;
  ...
  const registerItem = React50.useCallback((id, itemRef) => {
    setItems((prevItems) => [...prevItems, {
      id,
      ref: itemRef
    }]);
  }, []);
  const unregisterItem = React50.useCallback((id) => {
    setItems((prevItems) => prevItems.filter((i) => i.id !== id));
  }, []);
  const onItemKeyDown = React50.useCallback((event) => {
    var _a;
    if (!focusableItemId) {
      return;
    }
  ...
  const onItemFocus = React50.useCallback((id) => {
    if (focusableItemId !== id) {
      setFocusableItemId(id);
    }
  }, [focusableItemId, setFocusableItemId]);
  const onItemDisabled = React50.useCallback((id) => {
    var _a;
    const sortedItems = getSortedItems();
...
function useGlobalId2(idOverride) {
  const [defaultId, setDefaultId] = React51.useState(idOverride);
  const id = idOverride || defaultId;
  React51.useEffect(() => {
    if (defaultId == null) {
      globalId3 += 1;
...
var getTemplateColumns = (hideLegend, direction, position, width) => {
  if (direction === "vertical") {
    if (hideLegend) {
      return "1fr";
    }
...
var getTemplateRows = (hideLegend, direction) => {
  if (direction === "vertical") {
    if (hideLegend) {
      return "auto";
    }
...
true ? ChartsWrapper.propTypes = {
   ...
   */
  hideLegend: import_prop_types16.default.bool.isRequired,
  /**
...
//# sourceMappingURL=chunk-UIZF6VEE.js.map
